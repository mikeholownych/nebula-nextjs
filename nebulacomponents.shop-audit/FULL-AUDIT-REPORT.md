...
Domain not yet present in Common Crawl's web graph and no Moz/Bing API credentials configured — this is expected for a young domain, not a defect. One zero-cost fix remains: the founder's GitHub profile website field points to `aisyndicate.io` instead of `nebulacomponents.shop`. (The apparent LinkedIn company-page 404 was a wrong-slug testing artifact, not a real broken page — the correct URL is `linkedin.com/company/nebulacomponents`, confirmed valid and fixed in all internal references.) Realistic near-term link-building angles (submitting the open-source `citable` CLI to dev-tool directories, Show HN, guest posts on PPC/CRO publications) are detailed in `findings/backlinks.md`.
