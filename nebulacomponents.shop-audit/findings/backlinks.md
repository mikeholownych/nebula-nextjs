...
**5. LinkedIn company page URL (`linkedin.com/company/nebula-components`) returned HTTP 404 on direct check**
- **Evidence**: `verify_backlinks.py` recorded `status: lost, http_status: 404` for this exact URL. This is a directly observed HTTP response, not an inference about intent — it's possible the canonical company-page slug differs from what was checked, the page is unpublished, or it doesn't exist yet. It was not possible to distinguish between those from this response alone.
- **Recommendation**: Confirm the correct, canonical LinkedIn company page URL manually and ensure its "Website" field points to `nebulacomponents.shop`. If no company page exists yet, creating one is low-effort and gives a stable, permanent (if nofollowed) profile link plus a venue for building-in-public posts to attract inbound shares/links.
