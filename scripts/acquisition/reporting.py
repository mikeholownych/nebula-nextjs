...
def render_baseline_report(
    ...
    md.append(f"| Positions 1-10 | {meas['pos_bucket_1_10']} | {meas['pos_bucket_1_10'] / tot_p * 100:.1f}% |")
    md.append(f"| Positions 11-20 | {meas['pos_bucket_11_20']} | {meas['pos_bucket_11_20'] / tot_p * 100:.1f}% |")
    md.append(f"| Positions 21-30 | {meas['pos_bucket_21_30']} | {meas['pos_bucket_21_30'] / tot_p * 100:.1f}% |")
    md.append(f"| Positions 31-50 | {meas['pos_bucket_31_50']} | {meas['pos_bucket_31_50'] / tot_p * 100:.1f}% |")
    md.append(f"| Positions 51+ | {meas['pos_bucket_51_plus']} | {meas['pos_bucket_51_plus'] / tot_p * 100:.1f}% |")
    md.append("")
...
def render_weekly_observation_report(
    current_meas_id: str,
    prev_meas_id: Optional[str] = "meas_20260902_baseline_v2",
    db_uri: str = DEFAULT_DB_URI,
) -> str:
    """Render factual Weekly Acquisition Observation Report."""
    with psycopg.connect(db_uri, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
    ...
    md.append(f"**Effective Period:** {curr['effective_period_start']} to {curr['effective_period_end']} ({curr['window_days']} days)")
    ...
    md.append("## 2. Quantitative Search Visibility & Traffic Observations")
    md.append("")
    md.append("| Metric | Current Value | Baseline Delta | Description |")
    md.append("|--------|---------------|----------------|-------------|")
    ...
    if trend:
        md.append("## 3. Deterministic Trend Evaluation")
        md.append("")
    ...
    # 4. Search Visibility State Transitions
    ...
    md.append("## 4. Search Visibility State Transitions")
    md.append("")
    if prog_trans:
        md.append("### Progressions (Rank/Visibility Improvements)")
        for t in prog_trans:
            md.append(f"- **{t['canonical_url']}:** `{t['from_state']}` -> `{t['to_state']}` ({t['transition_reason']})")
        md.append("")
    if reg_trans:
        md.append("### Regressions (Rank/Visibility Declines)")
        for t in reg_trans:
            md.append(f"- **{t['canonical_url']}:** `{t['from_state']}` -> `{t['to_state']}` ({t['transition_reason']})")
        md.append("")
    if not prog_trans and not reg_trans:
        md.append("No state progressions or regressions detected in this period.")
        md.append("")

    # 5. Active Anomalies
    if curr["known_anomalies"]:
        md.append("## 5. Active Known Anomalies & Attribution Notes")
        md.append("")
    ...
    md.append("---")
    md.append("*Phase 4 Factual Observation Report. Recommendations intentionally deferred to Phase 5/6.*")
    return "\n".join(md)
