#!/usr/bin/env python3
"""Send the Friday GTM surge through the governed gate. Cap 3."""
...
BATCH = ROOT / "ops" / "friday_surge_20260819.json"
...
def main() -> int:
    ...
    if not dry_run and beta_setting("MAILCHECK_BETA_ENABLED") != "1":
        ...
        return 2
    max_prospects = min(3, max(1, int(beta_setting("MAILCHECK_BETA_MAX_PROSPECTS", "3"))))
    sent = 0
    blocked = 0
    processed = 0
    ...
    for prospect in batch["prospects"]:
        ...
        if existing:
            print(f"blocked_existing_lead: {email} | stage={existing.get('stage')}")
            blocked += 1
            continue
        subject = prospect.get("subject") or prospect["message"].splitlines()[0]
        if dry_run:
            print(f"DRY_RUN: {email} | {subject}")
            continue
        ...
        db.upsert_lead(
            ...
            url=prospect["site"],
            stage="discovered",
            source="friday_surge_20260819",
            trigger_context=prospect["trigger"],
            notes=json.dumps(
                {
                    "source_url": prospect.get("source_url"),
                    "audit_score": prospect.get("audit_score"),
                    "finding": prospect.get("finding"),
                }
            ),
        )
        client_id = "marketing:friday-surge:20260819:" + prospect["name"].lower().replace(" ", "-")
        hunter_result = hunter.verify(email)
        print(
            f"hunter_observation: {prospect['name']} | result={hunter_result.get('result', '')} | score={hunter_result.get('score', '')}"
        )
        try:
        ...
        if result.get("_error"):
            ...
            blocked += 1
            continue
        db.advance_stage(email, "contacted", source="friday_surge_20260819")
        try:
            mc.record_outcome(
        ...
        print(f"sent: {email} | provider_message_id={result.get('message_id') or result.get('id', '')}")
        sent += 1

    print(json.dumps({"sent": sent, "blocked": blocked, "dry_run": dry_run, "processed": processed}))
    return 0
