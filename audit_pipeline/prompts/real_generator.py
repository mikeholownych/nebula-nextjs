...
_SYSTEM_WRITER = (
    ...
    "- Use a goal → verify → constraints structure (the 'for/g, verify, "
    "constraints' completion contract): state what must be true when done, "
    "how to confirm it happened, and what NOT to do.\n"
    ...
    "For facts an audit cannot know, instruct the agent to ask the user for "
    "them (e.g. 'ask the user for a real review count') rather than fabricate.\n"
    "- The prompt should tell the agent exactly what to produce, the "
    "constraints (length, format, tone), the files to touch, and the "
    "verification commands.\n"
    "- Keep the prompt under ~550 words but specific enough to get a "
    "high-quality result on the first run.\n"
