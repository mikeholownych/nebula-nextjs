...
- Lighthouse remains a lab gate, not a production claim. Application-level hydration, prefetch, and route-wide webfont sources were removed from the audited shell without weakening the fixed budgets. All four LCP medians now pass locally; the remaining Next.js App Router runtime still exceeds the 200 ms TBT budget on three routes on the current verification host. A webpack control build reproduced the excess on the representative article, confirming that it is not isolated to the Learning Centre directory implementation.
