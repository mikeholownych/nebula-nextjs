...
- Slice 8 proof - real regression 2026-08-01: created public/monitor-qa.html (good: headline/CTA/social proof) → baseline 70/100 → replaced with stripped page (no CTA/headline) → watchdog emitted `📉 Nebula Monitor - https://nebulacomponents.com/monitor-qa / 70 → 45 (-25 pts)` and wrote event (regressed, prev 70, new 45) → cleanup (page + monitor + audits removed). Alert path proven end-to-end.
...
- Test account: e2e-crawler-test@example.com has 3 audits (nebulacomponents.com ×2 at 7.5, example.com at 5.5) + 8 rec cards - use it for demo/QA.
