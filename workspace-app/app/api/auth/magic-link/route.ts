...
export async function POST(request: NextRequest) {
  try {
    ...
    // Tag the surface so the platform API points the email's verify link at
    // this host (the session cookie must land on app.nebulacomponents.com).
    let body = rawBody
    try {
      const parsed = JSON.parse(rawBody)
      if (parsed && typeof parsed === 'object' && !parsed.surface) {
        parsed.surface = 'app'
