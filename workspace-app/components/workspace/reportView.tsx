...
import { SIGNAL_GROUPS } from '@/lib/reportArchitecture'
...
export default function ReportView({ audits }: { audits: WorkspaceAudit[] }) {
  ...
  const copyLink = () => {
    const url = `https://nebulacomponents.com/audit/${selectedId}/results`
  ...
  if (audits.length === 0) {
    return (
      <div className="py-16 text-center text-fg-muted">
        No audits yet. Run a{' '}
        <a href="https://nebulacomponents.com/audit?utm_source=workspace&utm_medium=internal" className="text-accent hover:underline">free audit</a>{' '}
      ...
      {detail && !loading && (
        <div className="rounded-md border border-border bg-bg-elevated p-8 print:border-none print:bg-white print:text-bg print:p-0">
          ...
          <div className="mt-10 border-t border-border print:border-gray-200 pt-6 text-xs text-fg-dim print:text-fg-muted flex items-center justify-between">
            <span>Generated by Nebula Components · nebulacomponents.com</span>
