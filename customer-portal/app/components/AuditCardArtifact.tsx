 ...
 * - Top leak callout (subtle danger border)
...
const GRADE_COLORS: Record<string, string> = {
  A: 'bg-accent/20 text-accent border border-border-accent/40',
  B: 'bg-accent-dim text-accent border border-border-accent/30',
  C: 'bg-signal-fail/20 text-signal-fail border border-signal-fail/30',
  D: 'bg-danger-dim text-danger border border-danger/30',
  F: 'bg-danger/20 text-danger border border-danger/40',
}
    ...
    >
      ...
      <div className="mb-4 flex items-center gap-2 rounded-lg bg-bg/50 px-3 py-2 text-xs text-fg-muted">
        <div className="flex gap-1">
          ...
          <span className="h-2 w-2 rounded-full bg-signal-fail/60" />
          <span className="h-2 w-2 rounded-full bg-accent/60" />
        </div>
        <span className="ml-2 flex-1 truncate font-mono text-[10px] text-fg-muted">{url}</span>
      </div>
      ...
      <div className="flex items-start gap-4">
        <div className="flex flex-col items-center">
          <motion.span
            ...
            className={`font-mono text-5xl font-bold tracking-tight tabular-nums ${
              score >= 80 ? 'text-accent' : score >= 60 ? 'text-accent' : 'text-signal-fail'
            }`}
          >
            {displayScore}
          </motion.span>
        ...
        <div className="flex-1">
          ...
          <motion.div
            ...
            className="mt-3 flex items-start gap-2 rounded-lg border border-danger/30 bg-danger-dim px-3 py-2"
          >
            <svg className="mt-0.5 h-4 w-4 flex-shrink-0 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {signals.map((signal, i) => (
          <motion.div
            ...
            transition={{ delay: 0.2 + i * 0.03, duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
            className={`
