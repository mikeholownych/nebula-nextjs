...
export default function DomainSettings() {
  ...
  return <section>
    ...
    <div className="rounded-xl border border-border bg-bg-elevated px-5 py-5 space-y-4">
      ...
      <div className="flex flex-wrap gap-2"><input id="custom-domain-hostname" aria-label="Custom domain hostname" value={hostname} onChange={(e) => setHostname(e.target.value)} placeholder="audits.example.com" className="min-w-0 flex-1 rounded-lg border border-border bg-bg-panel px-3 py-2 text-sm text-fg" /><button onClick={() => add(false)} disabled={!hostname.trim()} className="rounded-lg border border-border px-4 py-2 text-sm font-semibold text-fg disabled:opacity-50">Add custom domain</button></div>
