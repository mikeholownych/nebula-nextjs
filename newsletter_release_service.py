...
async def send_release(release: dict[str, Any], *, dry_run: bool = False, db_url: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        for subscriber in subscribers:
            ...
            result = await __import__("asyncio").to_thread(client.send, [current["email"]], release["subject"], text=payload["text"], html=payload["html"], client_id=f"newsletter:{key}", labels=["newsletter", release["campaign_id"]], headers=payload["headers"])
            async with pool.acquire() as conn:
