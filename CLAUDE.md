...
- Services: Next.js on localhost:3000 (systemd `nebula-nebula-nextjs` → `nebula-nextjs.service`), FastAPI platform API on localhost:8001 (systemd `nebula-platform-api.service`, venv via uv). Port 8765 is RESERVED-EMPTY: a stray repo-root http.server there historically exposed secrets (SEC-P0-2); the Hermes watchdog now alerts if anything binds it. Never start servers from the repo root.
...
- Drift surfaces: `$97`, `48 hours`, `9 conversion signals` - verify against canonical before commit
