...
**Date:** 2026-08-14
**Author:** ops-finance agent (task t_f9122dff)
**Task type:** CEO action — read-only verification
**Constraint:** No production changes made. No Stripe events triggered. No DB writes.
**Prior runs:** t_aa10d2ec (Aug 13), t_c165e519 (Aug 12), t_0b813c19 (Aug 11), t_31d5ea3f (Aug 10), t_74a37665 (Aug 9), t_34cf059c (Aug 8)

---

## Executive Summary

**Fix-pack ($97) path: VERIFIED end-to-end — unchanged from Aug 13.**
**Agency partner ($497) path: VERIFIED end-to-end — unchanged from Aug 13.**
**Subscription path ($29–$197/mo): STILL BLOCKED — schema defect now unresolved for 6 consecutive days (Aug 8–14).**
**Revenue to date: $0 real.**

The subscription schema defect (9 columns missing from `nebula_platform.subscriptions`) has not
been remediated. 8 commits were merged since Aug 13 — all are conversion/portal/data-rights
changes (ICP friction reduction, asset portability, competitor analysis portal components).
None touch `nebula_platform.subscriptions`. Any real subscription payment will fail at DB write
with a PostgreSQL error.

**This is day 6 of a known blocking defect on the subscription revenue path.**
...
| STRIPE_WEBHOOK_SECRET | CONFIRMED — `whsec_*` in systemd stripe.conf | systemd drop-in, confirmed this run |
...
| PostgreSQL cluster | ACTIVE — port 5433, `nebula_platform` online | psql connects, DB responds |
| deliver_prompt_pack.py | EXISTS — 20,098 bytes, Aug 11 | `ls -la /home/mike/nebula/scripts/deliver_prompt_pack.py` |

**No infrastructure regressions since Aug 13.**
...
| # | Fact | Source |
|---|------|--------|
| 1 | `purchases` table: 15 columns, `livemode BOOLEAN NOT NULL DEFAULT TRUE` present | prior: `\d purchases` on nebula_platform |
| 2 | `idx_purchases_live_email` index targets `livemode=true` rows | prior: `\d purchases` indexes |
| 3 | `pg_advisory_lock` + `ON CONFLICT (stripe_session_id) DO NOTHING` — idempotent | route.ts lines ~257, ~266 |
| 4 | QA fixture: `cs_test_billing_qa`, `e2e-crawler-test@example.com`, `fulfillment_status=delivered` | `SELECT COUNT(*) FROM purchases WHERE livemode=true` → 1 (QA only) |
| 5 | 1 total purchase row (0 real, 1 QA fixture) | verified this run: psql query |
| 6 | `deliver_prompt_pack.py` exists, 20,098 bytes, Aug 11 | `ls -la` — confirmed this run |
...
## 4. Subscription Path ($29–$197/mo) — BLOCKING DEFECT PERSISTS (Day 6)

### Status change from Aug 13

**None.** Schema defect identical. 8 commits since Aug 13 are conversion/portal/data-rights changes.
...
Result: 8 columns only — same as every day Aug 8–14.

### Defect (unchanged since Aug 8)

`route.ts` (lines 479–523, confirmed unchanged this run) INSERTs into `nebula_platform.subscriptions` with columns:
...
| Legacy `webhook_server.py` / nebula-webhook.service | INACTIVE | verified prior runs |
...
## 7. New Activity Since Aug 13

8 commits merged Aug 13–14. All are conversion/portal/data-rights changes:

- fix(conversion): eliminate ICP friction by clarifying CMS compatibility
- fix(data-rights): wrap cookies() call in try-catch for unit test context safety
- fix(data-rights): correct NextResponse import path from next/server
- feat(data-rights): add asset portability export API for zero hostage-taking compliance
- Merge feat/competitor-analysis-and-unfair-advantage-components into main
- feat(portal): add StackTaxComparison, HonestyGrid, AgenticNativeBanner, Citable Generator & CI conversion checker
- feat: add paid traffic leak report workflow
- docs: specify paid traffic leak report workflow

**None of the 8 commits touch:**
...
Active development signal: 8 commits in 1 day on site conversion and portal features — no
engineering attention directed at the schema defect despite 6 consecutive days of escalation.
...
DAYS SUBSCRIPTION BLOCKED: 6 (first verified Aug 8, unresolved through Aug 14)
NEW COMMITS SINCE AUG 13:  8 (all conversion/portal/data-rights — none touch payment paths)
...
| Subscription ($29–$197/mo) | **BLOCKED — SCHEMA DEFECT DAY 6** | CERTAIN | CEO: apply ALTER TABLE to nebula_platform NOW |
| Stripe dashboard config | **UNVERIFIED** | N/A | CEO: 1-min dashboard check |

**Critical escalation: The subscription schema defect has persisted for 6 consecutive days
(Aug 8–14) with zero remediation. All subscription revenue is at zero capture probability.
Development velocity is high (8 commits/day) but directed entirely at conversion and portal
features — not at the known revenue-blocking defect. The ALTER TABLE fix is a 30-second
operation with zero risk. No real subscribers yet — the window to fix before live revenue
loss remains open, but every day this goes unfixed is a day closer to a paying subscriber
hitting a silent $0 capture.**
...
| 2 | Webhook endpoint reachable, POST-only | `curl https://nebulacomponents.com/api/webhooks/stripe` → 405 — confirmed this run |
| 3 | Next.js service active, since Aug 12 07:42 UTC | `systemctl status nebula-nextjs` — confirmed this run |
| 4 | Cloudflare tunnel active, PID 2448120 | `pgrep -a cloudflared` — confirmed this run |
| 5 | `purchases` schema correct (15 cols, livemode present) | prior: `\d purchases` on nebula_platform |
| 6 | `subscriptions` schema DEFECTIVE (8 cols, missing 9) | `\d subscriptions` on nebula_platform — verified live this run |
| 7 | 0 subscriptions, 1 QA fixture purchase | psql COUNT queries — confirmed this run |
| 8 | route.ts INSERT uses 9 missing columns (lines 479–523) | `/home/mike/nebula/customer-portal/app/api/webhooks/stripe/route.ts` — grep confirmed unchanged |
| 9 | 8 new commits since Aug 13 — none touch subscriptions or payment routes | `git log --oneline --since="2026-08-13"` — confirmed this run |
| 10 | `deliver_prompt_pack.py` exists, 20,098 bytes | `ls -la /home/mike/nebula/scripts/deliver_prompt_pack.py` — confirmed this run |

---

*Report generated by ops-finance agent, task t_f9122dff. Read-only verification. No production changes made.*
*Prior reports: /home/mike/nebula/content/ops-finance/2026-08-13_revenue-funnel-reconciliation.md*
