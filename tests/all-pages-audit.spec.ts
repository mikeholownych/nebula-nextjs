...
// Generated by scripts that walk the repo (tests/_pagelist.json).
const PAGES: string[] = JSON.parse(readFileSync(__dirname + '/_pagelist.json', 'utf8'));
...
for (const pagePath of PAGES) {
  test(`PAGE ${pagePath}: visible + WCAG AA contrast`, async ({ page }) => {
    ...
    await page.goto(BASE_URL + pagePath + '?audit=' + Date.now(), { waitUntil: 'load', timeout: 30000 });
    ...
    const realErrors = pageErrors.filter(e => !e.includes('rb2b') && !e.includes('ERR_NAME_NOT_RESOLVED'));
    console.log(`[${pagePath}] VISIBILITY hidden=${vis.length} CONTRAST failures=${contrastFails} PAGE_ERRORS=${realErrors.length}`);
    if (vis.length) console.log(`[${pagePath}] HIDDEN: ` + vis.slice(0, 10).map((v: any) => `${v.tag}@${v.op} "${v.text}"`).join(' | '));
    if (realErrors.length) console.log(`[${pagePath}] ERRORS: ` + realErrors.slice(0, 5).join(' | '));

    expect(vis.length, `${pagePath} has hidden elements`).toBe(0);
    expect(contrastFails, `${pagePath} contrast failures`).toBe(0);
    expect(realErrors, `${pagePath} page errors`).toHaveLength(0);
  });
}
